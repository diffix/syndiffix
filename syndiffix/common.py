...
def check_column_names_or_ids(df: pd.DataFrame, columns: str | ColumnId | list[ColumnId | str]) -> None:
    """
    ...
    if isinstance(columns, str):
        ...
        if not (0 <= columns < num_columns):
            raise ValueError(f"ColumnId {columns} is out of range. DataFrame has {num_columns} columns (valid range: 0-{num_columns-1})")
    # Handle lists
        ...
        if all(isinstance(col, str) for col in columns):
            ...
            if invalid_columns:
                raise ValueError(f"Column names {invalid_columns} not found in DataFrame. Available columns: {list(df.columns)}")
        ...
        # Check if all elements are ColumnIds (integers)
            ...
            if invalid_ids:
                raise ValueError(f"ColumnIds {invalid_ids} are out of range. DataFrame has {num_columns} columns (valid range: 0-{num_columns-1})")
