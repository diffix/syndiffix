...
def test_quality_float_str() -> None:
    ...
    category_indices = np.clip(
        (normalized_float * 8 + np.random.normal(0, 1, num_rows)).astype(int), 
        0, 9
    )
        ...
        if count < min_count:
            ...
            if not rows.empty:
                ...
                new_rows = pd.DataFrame({
                    "float_col": np.random.normal(50, 15, min_count),
                    "str_col": [cat]*min_count
                })
...
def test_quality_str_str() -> None:
    ...
    assert syn_cramers_v == approx(orig_cramers_v, rel=0.4), f"Cramér's V differs too much: orig={orig_cramers_v}, syn={syn_cramers_v}"
